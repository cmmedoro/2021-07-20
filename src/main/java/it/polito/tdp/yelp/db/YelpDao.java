...
public class YelpDao {
	...
	public List<User> getUsersWithReviews(int minReviews){
		...
		try {
			...
			ResultSet res = st.executeQuery();
			while (res.next()) {
				User user = new User(res.getString("user_id"), res.getInt("votes_funny"), res.getInt("votes_useful"), res.getInt("votes_cool"),
						res.getString("name"),
	...
	public int calcolaSimilarita(User u1, User u2, int anno) {
		String sql = "SELECT COUNT(*) AS similarita "
				...
				+ "AND r1.user_id = ? AND r2.user_id = ? AND YEAR(r1.review_date) = ? AND YEAR(r2.review_date) = ?";
		Connection conn = DBConnect.getConnection();
